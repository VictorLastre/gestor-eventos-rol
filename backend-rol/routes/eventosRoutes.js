...
const registrarLog = (usuario, accion, descripcion) => {
  ...
  db.query(sql, [usuario.id, usuario.nombre, accion, descripcion], (err) => {
    if (err) console.error("âŒ Error en bitácora:", err);
  });
};
...
router.get('/:id/partidas', verificarToken, (req, res) => {
  const sql = `
    SELECT 
      ...
      p.cupo, p.turno, p.estado, p.etiqueta, p.apta_novatos, p.para_infancias, p.materiales_pedidos, p.continuacion_de_id,
      ...
      (SELECT estado FROM inscripciones WHERE partida_id = p.id AND usuario_id = ?) AS mi_estado,
      (1 + (
        SELECT COUNT(*) 
        FROM partidas ph 
      ...
      (SELECT COUNT(p2.id) FROM partidas p2 JOIN eventos e2 ON p2.evento_id = e2.id WHERE p2.dungeon_master_id = p.dungeon_master_id AND e2.estado = \'Finalizado\') AS dm_honor, IFNULL((SELECT SUM(r.voto) FROM reputacion r JOIN partidas p2 ON r.partida_id = p2.id WHERE r.evaluado_id = p.dungeon_master_id AND p2.dungeon_master_id != p.dungeon_master_id), 0) AS dm_reputacion_neta
    FROM partidas p 
...
router.post('/:id/partidas', verificarToken, (req, res) => {
  ...
  const { titulo, descripcion, requisitos, sistema, sistema_id, cupo, turno, etiqueta, apta_novatos, materiales_pedidos, codigo_privado, continuacion_de_id } = req.body;
  ...
  db.query(sqlCheck, [eventoId, eventoId, eventoId, usuarioId, eventoId, usuarioId, eventoId, usuarioId, eventoId], (err, resultados) => {
    ...
    const sqlInsert = `
        ...
        (evento_id, dungeon_master_id, titulo, descripcion, requisitos, sistema, sistema_id, cupo, turno, estado, etiqueta, apta_novatos, materiales_pedidos, codigo_privado, continuacion_de_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'abierta', ?, ?, ?, ?, ?)
    `;
    
    db.query(sqlInsert, [eventoId, usuarioId, titulo, descripcion, requisitos, sistema, sistema_id, cupo, turno, etiqueta, apta_novatos, materiales_pedidos, codigo_privado || null, continuacion_de_id || null], (err, resultadoPartida) => {
      if (err) {
        console.error(err);
        return res.status(500).json({ error: 'Error al crear la mesa.' });
      }
      
      if (continuacion_de_id) {
        db.query("SELECT usuario_id FROM inscripciones WHERE partida_id = ? AND (estado = 'confirmada' OR estado IS NULL)", [continuacion_de_id], (err, oldPlayers) => {
          if (!err && oldPlayers && oldPlayers.length > 0) {
            const values = oldPlayers.map(p => [p.usuario_id, resultadoPartida.insertId, 'pendiente']);
            db.query("INSERT INTO inscripciones (usuario_id, partida_id, estado) VALUES ?", [values], (err) => {
              if (err) console.error("Error al precargar jugadores:", err);
            });
          }
        });
      }
