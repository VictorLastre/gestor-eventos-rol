...
router.get('/notificaciones', verificarToken, (req, res) => {
  ...
  router.get('/', verificarToken, (req, res) => {
    ...
    db.query("SELECT COUNT(*) AS total FROM usuarios", (err, countResult) => {
      if (err) return res.status(500).json({ error: 'Error.' });
      const sql = `SELECT id, nombre, nombre_completo, email, rol, avatar, biografia, solicita_dm, es_dm_nuevo, telegram_chat_id, IFNULL((SELECT SUM(voto) FROM reputacion WHERE evaluado_id = usuarios.id), 0) as honor_total, IFNULL((SELECT SUM(voto) FROM reputacion WHERE evaluado_id = usuarios.id), 0) as reputacion_neta FROM usuarios ORDER BY nombre ASC LIMIT ${limit} OFFSET ${offset}`;
      db.query(sql, (err, resultados) => {
        if (err) return res.status(500).json({ error: 'Error.' });
  ...
  router.get('/yo', verificarToken, (req, res) => {
    db.query("SELECT id, nombre, nombre_completo, email, rol, avatar, biografia, solicita_dm, es_dm_nuevo, telegram_chat_id, IFNULL((SELECT SUM(voto) FROM reputacion WHERE evaluado_id = usuarios.id), 0) as honor_total, IFNULL((SELECT SUM(voto) FROM reputacion WHERE evaluado_id = usuarios.id), 0) as reputacion_neta FROM usuarios WHERE id = ?", [req.usuario.id], (err, resultados) => {
  ...
  router.get('/:id/perfil_publico', verificarToken, (req, res) => {
    ...
    const sqlPerfil = `
      ...
      IFNULL((SELECT SUM(voto) FROM reputacion WHERE evaluado_id = usuarios.id), 0) as honor_total,
