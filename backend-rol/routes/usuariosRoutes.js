...
router.get('/notificaciones', verificarToken, (req, res) => {
  ...
  router.get('/:id/perfil_publico', verificarToken, (req, res) => {
    ...
    const sqlPerfil = `
      ...
      ((SELECT COUNT(*) FROM honor_dm WHERE dm_id = usuarios.id) + ((SELECT COUNT(p2.id) FROM partidas p2 JOIN eventos e2 ON p2.evento_id = e2.id WHERE p2.dungeon_master_id = usuarios.id AND e2.estado = 'Finalizado') * 5)) as honor_total 
