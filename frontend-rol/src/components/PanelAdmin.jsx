...
function PanelAdmin({ setVista }) {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 md:p-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* ðŸ‘‘ ENCABEZADO DE LA MESA DE COMANDO */}
      ...
      {/* ðŸ° PANEL DE NAVEGACIÃ“N Y CONTENEDOR */}
      {/* AjustÃ© los bordes redondeados en mÃ³vil para que no ocupen tanto espacio */}
      <section className="bg-zinc-900 rounded-[2rem] md:rounded-[3rem] border border-purple-500/20 shadow-2xl overflow-hidden flex flex-col">
        ...
        {/* Ajuste de padding y gap en el nav para mÃ³viles */}
        <nav className="flex flex-col sm:flex-row bg-zinc-950/80 p-2 sm:p-3 gap-2 border-b border-zinc-800 w-full">
          {['eventos', 'usuarios', 'stats'].map(tab => (
            <button 
              ...
              title={tab === 'eventos' ? 'Gestor de Jornadas' : tab === 'usuarios' ? 'Alistamiento' : 'OrÃ¡culo de Datos'}
            >
              {tab === 'eventos' ? 'âš”ï¸ Gestor de Jornadas' : tab === 'usuarios' ? 'ðŸ›¡ï¸ Alistamiento' : 'ðŸ“Š OrÃ¡culo de Datos'}
            </button>
          ))}
        </nav>
        
        {/* overflow-x-hidden aÃ±adido por seguridad para evitar el scroll horizontal interno */}
