...
  // ✨ AQUÍ ESTÁ LA MAGIA: Un estado dinámico para los turnos
  const [turnos, setTurnos] = useState([
    { hora_inicio: '16:00', hora_fin: '16:45' } // Un turno por defecto para guiar al usuario
  ]);
...
  // Funciones para manejar la lista dinámica de turnos
...
    e.preventDefault();
    
    // Validación básica de turnos vacíos
...
      const res = await fetchProtegido(`https://hotpink-butterfly-694113.hostingersite.com/api/escapes/${eventoId}`, {
        method: 'POST',
        body: JSON.stringify({ ...formData, turnos }) // Enviamos los datos + el array de turnos
      });
...
                      required
                      className="flex-1 bg-zinc-900 border border-zinc-800 text-white text-sm font-bold rounded-xl py-3 px-4 focus:border-indigo-500 outline-none" 
                    />
...
                    className="w-full sm:w-12 h-12 bg-zinc-900 hover:bg-red-500/20 text-zinc-500 hover:text-red-500 border border-zinc-800 hover:border-red-500/50 rounded-xl flex items-center justify-center transition-all"
...
              ))}
            </div>
            <p className="text-[10px] text-zinc-500 italic mt-2 ml-2">💡 Tip: Deja un margen de 10-15 min entre pases para rearmar los acertijos.</p>
          </div>
